...
class ChatDataset(Dataset):
    def __init__(
        ...
        for record in tqdm(original_records):
            if random.random() > self.sample_rate:
                continue
            tensors = self.convert_record(record, column=column)
            if tensors is None:
                continue
    ...
    def convert_record(self, record, column="messages"):
        input_ids, labels = [], []

        for i, message in enumerate(record[column]):
            if message['role'] == 'bot':
                message['role'] = 'assistant'
                record[column][i]['role'] = 'assistant'
            if len(message['content']) > 15000:
                return None
        ...
        original_input_ids = self.get_tokens(record[column])
