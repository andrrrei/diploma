...
def load_grandmaster_dataset(
    dataset_name, sample_fraction=1.0, max_answer_tokens=10000
):
    ...
    print(f"Оригинальный размер train: {len(train_dataset)}")
    print(f"Оригинальный размер test: {len(test_dataset)}")

    train_dataset = train_dataset.filter(
        lambda x: x["answer_tokens"] <= max_answer_tokens, num_proc=4
    )
    ...
    print(f"После фильтрации: train = {len(train_dataset)}, test = {len(test_dataset)}")

    path = "datasets/GrandMaster-PRO-MAX"
...
train_file, val_file = load_grandmaster_dataset(
...
trainer = SFTTrainer(
    config_file="sft_gm/train_config_3b.json",
